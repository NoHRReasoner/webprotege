 ...
 */
public class NohrDBMappingsHtmlRenderer implements TreeNodeRenderer<NohrDBMapping> {
    ...
    @Inject
    public NohrDBMappingsHtmlRenderer(@Nonnull LoggedInUserProvider loggedInUserProvider,
    ...
    @Override
    public String getHtmlRendering(NohrDBMapping node) {
    ...
    private void renderIcon(NohrDBMapping node, StringBuilder sb) {
        String iconIri;
        DataResource icon = getIcon(node);
        sb.append("<img src='").append(icon.getSafeUri().asString()).append("'/>");
    }

    private void renderDisplayName(NohrDBMapping node, StringBuilder sb) {
    ...
    private void renderPrimaryDisplayName(NohrDBMapping node, StringBuilder sb) {

            sb.append(highlightText(node.getDbMapping()));
    ...
    private void renderSecondaryDisplayName(NohrDBMapping node, StringBuilder sb) {
        String secondaryText = node.getDbMapping();
    ...
    private void renderCommentsIcon(NohrDBMapping node, StringBuilder sb) {
    ...
    private void renderWatchesIcon(NohrDBMapping node, StringBuilder sb) {
    ...
    private void renderTags(NohrDBMapping node, StringBuilder sb) {
        /*if(!renderTags) {
            return;
        }
    ...
    private void renderNoDisplayName(NohrDBMapping node, StringBuilder sb) {
        sb.append("<span class='wp-entity-node__display-name__no-display-name' title='")
                .append(messages.displayName_noDisplayName_helpText(node.getDbMapping().toLowerCase(),
                        node.getDbMapping()))
                .append("'>");
    ...
    @Nonnull
    private DataResource getIcon(@Nonnull NohrDBMapping node) {
